...
// 방법 1. 함수 포인터
// => 캡쳐한 람다 표현식은 받을 수 없다.
// => 함수 포인터로 변환을 지원하지 않는 함수 객체를 받을 수 없다.
// => f 사용시 인라인 치환 안됨.
...
// 방법 2. std::function 사용
// 특징 1. f(1,2)가 인라인 치환은 안됨
//	   2. 호출가능한 모든 것을 받을 수 있다.
...
// => 람다표현식은 임시객체인데..
// => lvalue reference로는 받을 수 없다.
// template<typename T>
// void find_if(T& f){ f(1,2); }

// 방법 5. 템플릿 - const lvalue reference
// => operator() 가 상수 멤버 함수 인것만 호출할 수 있게 된다.
// => mutable 람다 표현식을 사용할 수 없다.
/*
template<typename T>
void find_if(const T& f){
	f(1,2); 		// f.operator()(1,2)에서 f는 상수 객체이므로 () 연산자 함수는 반드시 상수 함수이어야한다.
}
*/

// 방법 6. T&& forwarding reference
// => 나쁘지 않은 방법. 자주 사용!
// => 인자로 받게 되는 함수객체의 크기가 큰 경우!
template<typename T>
void find_if(T&& f){
	f(1,2);
}
...
int main()
{
	...
	find_if([v](int a, int b){return a-b+v;});
...
// 뭐가 좋나요? it's up to the situation
// find_if 라면 전달되는 함수 객체의 크기가 크지 않고
// 인라인 치환성이 중요 하므로 - T f !!!!

// cppreference.com std::find_if 찾아서 3번째 인자 확인해보세요. 1번 방식이네용