...
struct Plus
{
	...
	inline int operator()(int a, int b) const		// <= 뒤에 const를 붙이면 상수 멤버 함수라는 의미 상수성이 있을 때에도 부를 수 있는 멤버함수
	{
		return a + b;
	}
};
...
void foo(const Plus& f){
	int n = f(1,2); // f.operator()(1,2) 인데..
					// f 는 상수 객체 입니다.
					// "상수 객체는 상수 멤버함수만 호출가능" 하므로
					// 이 코드가 되려면 () 가 상수 멤버 함수이어야 합니다.
}


int main()
{
	Plus p; // p는 함수가 아니라 객체(구조체)입니다.!

	int n = p(1, 2);	 // 객체인 p를 함수 처럼 사용합니다. 
						// 함수 객체(function object)라고 합니다.
						// p.operator()(1,2)
